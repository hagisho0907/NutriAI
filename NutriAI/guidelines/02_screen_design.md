...
- **日付ナビゲーション** (2025-10-22更新):
  ...
  - 「今日」「昨日」「明日」を自動判定して表示
  - 全画面でカレンダーDialog（モーダル）形式に統一（Dialog + Calendar + CalendarIconボタン）
  - モバイル対応として画面中央にモーダル表示し、scale-90でサイズ調整してレイアウト崩れを防止
- **カロリー表示** (2025-10-17):
  - 円形プログレス表示（w-36 h-36）
...
- **日付ナビゲーション** (2025-10-22更新):
  - ChevronLeft/ChevronRightアイコンで前後の日付に移動可能
  - カレンダーDialog（モーダル）形式で日付選択（Dialog + Calendar + CalendarIconボタン）
  - 「今日」「昨日」を自動判定して表示
  - モバイル対応として画面中央にモーダル表示し、scale-90でサイズ調整してレイアウト崩れを防止
- **日次サマリー** (2025-10-17):
