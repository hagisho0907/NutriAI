...
| SCR-09 | プロフィール設定 (2025-10-22新規追加) | ユーザー情報変更と目標設定管理 | メイン設定画面、ユーザー情報編集画面、目標設定画面 | ダッシュボードのプロフィールアバターからアクセス |
...
- ボタン（2025-10-22更新）: 主要導線はフィルドボタン(緑ベース)、副次はアウトライン、危険操作は赤系。プライマリボタンのホバー時は深緑（#2F855A）、アウトラインボタンのホバー時は薄い緑背景と緑ボーダーで統一。「+ 追加」ボタンは`text-primary`のゴーストボタンで、ホバー時は背景色を変更せず文字を太くする控えめな効果（`hover:font-semibold`）を適用し、統一感のあるUIを実現。
...
- 背景色統一（2025-10-22更新）: 全画面でbg-primary/5（薄い緑）を統一背景色として使用。カードなどのコンテンツは白背景（bg-white）で、視覚的な階層構造を明確化。
...
### SCR-04 食事記録・MyFitnessPal風管理 (2025-10-17更新、2025-10-22更新)
...
[今日の歩数カード]
  📊 8,547歩
  距離: 6.0 km | 消費カロリー: 342 kcal

[デバイス連携カード]
  📱 Apple Health [スイッチON/OFF]
  📱 Google Fit [スイッチON/OFF]  
  ⌚ Fitbit [スイッチON/OFF]
  ⌚ Garmin [スイッチON/OFF]

[注意事項]
  • デバイス連携には各サービスでの認証が必要
  • 歩数データは運動記録の消費カロリー計算に反映
```
...
[食事リマインダーカード]  
  食事記録リマインダー [スイッチON/OFF]
  朝食: 08:00 | 昼食: 12:00 | 夕食: 19:00

[その他のリマインダーカード]
  🏃 運動リマインダー 18:00 [スイッチON/OFF]
  ⚖️ 体重測定リマインダー 07:00 [スイッチON/OFF]
  💧 水分摂取リマインダー 2時間間隔 [スイッチON/OFF]
...
- **機能詳細** (2025-10-22):
  - **メイン設定画面**: プロフィール概要表示 + 6つの設定メニュー
  - **ユーザー情報編集**: 基本情報、身体データ、活動レベル・目標の変更
  - **目標設定**: カロリー目標 + PFCバランス（プリセット4種類 + カスタム調整）
  - **歩数連携**: Apple Health、Google Fit、Fitbit、Garminとの連携設定
  - **表示設定**: ダークモード/ライトモード切り替え + アクセントカラー設定
  - **通知設定**: 食事・運動・体重等のリマインダー設定、静音時間設定
  - **ログアウト**: アプリからのログアウト機能
...
- データ構造: UserProfile、GoalSettings、StepCounterSettings、NotificationSettings、AppearanceSettings。
- 状態/バリエーション: 6つの画面間のナビゲーション、デバイス連携状態管理、テーマ切り替え、通知設定の詳細管理、ログアウト処理。
